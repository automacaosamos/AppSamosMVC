...
begin
  Handled := False;
  varGUID   := Context.LoggedUser.Roles[0];
  if not GetRedisClient.GET(varGUID, varJSON) then
    begin
      Render(401, 'Authorization Forbidden');
      Exit;
    end;
  Handled := not IsAuthorized(Self.ClassName, varJSON, Context.Request.HTTPMethod);
  if Handled then
    Render(401, 'Authorization Forbidden');
  inherited;
end;
