Ext.define('AppSamos.view.cidades.form.ViewController', {
    extend: 'Ext.app.ViewController',
    alias: 'controller.cidadesform',
    ...
    onIncluirClick: function(btn, e) {
        ...
        const novaEmpresas = Ext.create('AppSamos.view.cidades.Model');
    ...
    onGravarClick: async function(btn, e) {
        ...
        Utils.Msg.confirm('Quer realmente gravar ?', btn => {
            if(btn == 'yes') {
                ...
                proxy.setUrl(localStorage.getItem('api') + '/cidades');
                proxy.setHeaders({
                    'Authorization' : 'Bearer ' + localStorage.getItem('token')
                });

                model.set({
                    'CIDADES_STATUS'         : this.lookup('status').getValue()         ? 'T' : 'F'
                });

                if(isNaN(model.get('CIDADES_ID'))){
                    model.set('CIDADES_ID', '0');
                    model.phantom = true;
                }
                ...
                model.save({
                    success: (record, operation) => {
                        ...
                        model.set('CIDADES_ID', record.get('CIDADES_ID'));

                        const dialog = Ext.Msg.alert('Mensagem', 'Gravado com sucesso');
                        Ext.defer(dialog.hide, 2000, dialog);

                        const storeEmpresas = this.getView().up('cidadeslist').getViewModel().getStore('cidades');
            ...
            if(btn == 'yes') {
                ...
                model.erase({
                    success: (record, operation) => {
                        ...
                        this.getView().destroy();
                        const dialog = Ext.Msg.alert('Mensagem', 'Cidade excluída com sucesso');
                        Ext.defer(dialog.hide, 2000, dialog);
