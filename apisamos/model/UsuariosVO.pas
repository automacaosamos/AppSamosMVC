      ...
      if StrLeft(StrParametros[0],1) = '.' then
        ...
        begin
          ...
          case Valor_Int(StrParametros[2]) of
            ...
            1: SQLWhere := SQLWhere + 'and USUARIOS_NOME LIKE '     + QuotedStr('%' + StrParametros[0] + '%');
            2: SQLWhere := iif(StrEmpty(Parametros[4]),'','and USUARIOS_ID = ' + IntToStr(Valor_Int(StrParametros[0])));
          end;
          case Valor_Int(StrParametros[2]) of
            0: SQLOrder := ' order by USUARIOS_NOME';
            1: SQLOrder := ' order by USUARIOS_NOME';
            2: SQLOrder := ' order by USUARIOS_ID';
          end;
        end;
